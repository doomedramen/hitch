...
mod concurrent_operations_tests {
    ...
    fn test_rapid_multiple_environment_additions() -> Result<()> {
        with_test_env(SetupLevel::GitOnly, |test_env| {
            ...
            assert!(successful_envs.len() >= 4, "Expected at least 4 environments to be added successfully");
            ...
            for env in &successful_envs {
                assert!(status_stdout.contains(env), "Environment {} should exist in status", env);
            }
    ...
    fn test_rapid_promote_demote_operations() -> Result<()> {
        with_test_env(SetupLevel::GitOnly, |test_env| {
            ...
            for (i, branch_name) in feature_branches.iter().enumerate() {
                create_and_commit_file(test_env, &format!("feature{}.txt", i), &format!("Feature {} content", i))?;
                ...
                if promote_output.status.success() {
                    ...
                    if i > 0 {
                        let demote_output = run_hitch_command(test_env, &["demote", &successful_branches[i-1], "dev"])?;
            ...
            assert!(successful_branches.len() >= 1, "Expected at least 1 promotion to succeed");

            // Verify final state
            let status_output = run_hitch_command(test_env, &["status"])?;
            assert!(status_output.status.success(), "Status should work after rapid promote/demote operations");
        ...
        with_test_env(SetupLevel::GitOnly, |test_env| {
            ...
            for i in 0..5 {
                ...
                if rebuild_output.status.success() {
                    ...
                    if i < 3 {
                        create_and_commit_file(test_env, &format!("change{}.txt", i), &format!("Change {}", i))?;
                        ensure_clean_working_tree(test_env)?;
                    }
                }
            }
            ...
            assert!(successful_rebuilds >= 1, "Expected at least 1 rebuild to succeed");

            // Environment should still be in valid state
            let final_status = run_hitch_command(test_env, &["status"])?;
            assert!(final_status.status.success(), "Status command should work after rapid rebuilds");
    ...
    fn test_rapid_lock_unlock_operations() -> Result<()> {
        with_test_env(SetupLevel::GitOnly, |test_env| {
            ...
            for i in 0..3 {
                ...
                let rebuild_output = run_hitch_command(test_env, &["rebuild", "dev"])?;
                assert!(!rebuild_output.status.success(), "Normal rebuild should fail on locked environment");

                // Force rebuild (should succeed)
                let force_rebuild_output = run_hitch_command(test_env, &["rebuild", "dev", "--force"])?;
                assert!(force_rebuild_output.status.success(), "Force rebuild should succeed on locked environment");
                ensure_clean_working_tree(test_env)?;

                // Unlock environment
                let unlock_output = run_hitch_command(test_env, &["unlock", "dev"])?;
                assert!(unlock_output.status.success(), "Unlock {} should succeed", i);
                ensure_clean_working_tree(test_env)?;

                // Normal rebuild should work now
                let normal_rebuild_output = run_hitch_command(test_env, &["rebuild", "dev"])?;
                assert!(normal_rebuild_output.status.success(), "Normal rebuild should work after unlock");
                ensure_clean_working_tree(test_env)?;
            }

            // Final state should be consistent
            let final_status = run_hitch_command(test_env, &["status"])?;
            assert!(final_status.status.success(), "Status should work after rapid lock/unlock operations");
        ...
        with_test_env(SetupLevel::GitOnly, |test_env| {
            ...
            for _i in 0..10 {
                ...
                if status_output.status.success() {
                    ...
                    assert!(status_stdout.contains("dev"), "Status should always show dev environment");
                    assert!(status_stdout.contains("staging"), "Status should always show staging environment");
                    assert!(status_stdout.contains("prod"), "Status should always show prod environment");
                }
            }
    ...
    fn test_rapid_mixed_operations() -> Result<()> {
        with_test_env(SetupLevel::GitOnly, |test_env| {
            ...
            assert!(successful_operations >= 8, "Expected at least 8 operations to succeed");

            // Final state should be consistent
            let final_status = run_hitch_command(test_env, &["status"])?;
            assert!(final_status.status.success(), "Final status should be consistent");
            ...
            assert!(status_stdout.contains("staging"), "Should have staging environment");
            assert!(status_stdout.contains("prod"), "Should have prod environment");
    ...
    fn test_environment_creation_promotion_race() -> Result<()> {
        with_test_env(SetupLevel::GitOnly, |test_env| {
            ...
            assert!(add_success || promote_success, "At least one operation should succeed");
        ...
        with_test_env(SetupLevel::GitOnly, |test_env| {
            ...
            for i in 0..5 {
                ...
                if add_output.status.success() {
                    ...
                    let promote_output = run_hitch_command(test_env, &["promote", &branch_name, &env_name])?;
            ...
            let status_output = run_hitch_command(test_env, &["status"])?;
            assert!(status_output.status.success(), "Status should work after rapid metadata operations");
            ...
            assert!(branch_stdout.contains("hitch-metadata"), "hitch-metadata branch should exist after rapid operations");
