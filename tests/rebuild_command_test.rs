...
fn test_rebuild_missing_branch() -> Result<()> {
    with_test_env(SetupLevel::Complete, |test_env| {
        ...
        test_env.create_hitch_config_with_environment("dev", "main", &["nonexistent/feature"], false)?;
...
fn test_rebuild_multiple_branches() -> Result<()> {
    with_test_env(SetupLevel::Complete, |test_env| {
        ...
        for branch in &branches {
            test_env.create_branch_with_content(branch, &format!("{}.md", branch.replace("/", "_")), &format!("# {} feature\n", branch))?;
        }
        ...
        assert!(test_env.path().join("README.md").exists(), "dev branch should have base branch content");

        for branch in &branches {
            let filename = format!("{}.md", branch.replace("/", "_"));
            assert!(test_env.path().join(&filename).exists(), "dev branch should have content from {}", branch);
        }
...
"#;
        ...
        assert!(output.status.success(), "rebuild should succeed with git hooks");
        ...
        assert!(test_env.path().join("README.md").exists(), "dev branch should have base branch content");
        assert!(test_env.path().join("feature.txt").exists(), "dev branch should have feature branch content");
