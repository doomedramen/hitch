...
mod upstream_tracking_tests {
    ...
    fn test_force_push_sets_upstream_tracking() -> Result<()> {
        with_test_env(SetupLevel::GitOnly, |test_env| {
            ...
            assert!(!has_upstream_tracking(test_env, "dev")?,
            ...
            let _output = run_hitch_command_with_input(test_env, &["rebuild", "dev", "--replace-remote"], "y\n")?;
            ...
            assert!(tracking_info.contains("[origin/dev]"),
    ...
    fn test_promote_replace_remote_sets_upstream_tracking() -> Result<()> {
        with_test_env(SetupLevel::GitOnly, |test_env| {
            ...
            let _output = run_hitch_command_with_input(test_env, &["promote", "feature1", "dev", "--replace-remote"], "y\n")?;
    ...
    fn test_demote_replace_remote_sets_upstream_tracking() -> Result<()> {
        with_test_env(SetupLevel::GitOnly, |test_env| {
            ...
            let _output = run_hitch_command_with_input(test_env, &["demote", "feature1", "dev", "--replace-remote"], "y\n")?;
    ...
    fn test_multiple_environments_upstream_tracking() -> Result<()> {
        with_test_env(SetupLevel::GitOnly, |test_env| {
            ...
            assert!(!has_upstream_tracking(test_env, "dev")?, "Dev should not have upstream tracking");
            assert!(!has_upstream_tracking(test_env, "staging")?, "Staging should not have upstream tracking");
            assert!(!has_upstream_tracking(test_env, "prod")?, "Prod should not have upstream tracking");

            // Rebuild each environment with --replace-remote
            run_hitch_command_with_input(test_env, &["rebuild", "dev", "--replace-remote"], "y\n")?;
            run_hitch_command_with_input(test_env, &["rebuild", "staging", "--replace-remote"], "y\n")?;
            run_hitch_command_with_input(test_env, &["rebuild", "prod", "--replace-remote"], "y\n")?;

            // Verify all environments now have upstream tracking
            assert!(has_upstream_tracking(test_env, "dev")?, "Dev should have upstream tracking");
            assert!(has_upstream_tracking(test_env, "staging")?, "Staging should have upstream tracking");
            assert!(has_upstream_tracking(test_env, "prod")?, "Prod should have upstream tracking");
    ...
    fn test_force_push_failure_handling() -> Result<()> {
        with_test_env(SetupLevel::GitOnly, |test_env| {
            ...
            run_git_command(test_env, &["remote", "set-url", "origin", "invalid-url-that-will-fail"])?;
            ...
            run_git_command(test_env, &["remote", "set-url", "origin", test_env.path().to_str().unwrap()])?;
