...
- **Minimum supported git: 2.30.** `git update-ref --stdin -z` batches are atomic in every version at or above that floor; do not use the newer `start`/`prepare`/`commit` transaction verbs.
